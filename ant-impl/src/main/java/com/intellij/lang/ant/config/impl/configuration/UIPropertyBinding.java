...
public abstract class UIPropertyBinding {
  ...
  public static class Composite extends UIPropertyBinding {
    ...
    public RawCommandLineEditorBinding bindString(RawCommandLineEditor textComponent, AbstractProperty<String> property) {
      RawCommandLineEditorBinding binding = new RawCommandLineEditorBinding(textComponent, property);
      myBindings.add(binding);
      return binding;
    }
    ...
    public <T extends JDOMExternalizable> TableListBinding<T> bindList(JTable table,
                                                                       consulo.ui.ex.awt.ColumnInfo[] columns,
                                                                       ListProperty<T> property) {
  ...
  public static class RawCommandLineEditorBinding extends ComponentBinding<RawCommandLineEditor, AbstractProperty<String>> {
    private Disposable myListenerDisposable;

    public RawCommandLineEditorBinding(RawCommandLineEditor textComponent, AbstractProperty<String> property) {
      super(textComponent, property);
    }
    ...
    public void loadValues(AbstractProperty.AbstractPropertyContainer container) {
      ...
      myListenerDisposable = getComponent().addValueListener(valueEvent -> apply(container));
    }
  ...
  public static class TableListBinding<T> extends ComponentBinding<JTable, ListProperty<T>> {
    ...
    public TableListBinding(final JTable table, consulo.ui.ex.awt.ColumnInfo[] columns, ListProperty<T> property) {
      ...
      myModel = new consulo.ui.ex.awt.table.ListTableModel<T>(columns);
    ...
    public void loadValues(AbstractProperty.AbstractPropertyContainer container) {
      ...
      if (myModel.isSortable()) {
        final consulo.ui.ex.awt.ColumnInfo[] columnInfos = myModel.getColumnInfos();
        int sortByColumn = -1;
    ...
    public void addAddFacility(JButton addButton, final Supplier<T> factory) {
  ...
  public static class SortedListBinding<T extends JDOMExternalizable> extends BaseListBinding<T> {
    public SortedListBinding(JList list, ListProperty<T> property, Comparator<T> comparator) {
      super(property, list);
      list.setModel(new consulo.ui.ex.awt.SortedListModel<T>(comparator));
    }
  ...
  public static class OrderListBinding<T> extends BaseListBinding<T> {
    ...
    public void addAddManyFacility(JButton button, final Supplier<List<T>> factory) {
      button.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          List<T> items = factory.get();
